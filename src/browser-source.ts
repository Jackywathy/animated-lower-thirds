...
function stopTimeCount(i: string) {
    if (i == "lower-third-1"){
      c1 = 0;
      d1 = 0;
      active1IsOn = false;
...
function startInactiveCount(i: string) {
  ...
  function inactiveCount(i: string) {
    switch (i) {
        case "lower-third-1": {
...
function startActiveCount(i: string) {
...
function loadLogo(alt: string, logo: string = "") {
    if(logo){
        $('#'+alt).attr("src", logo);
    } else {
        $('#'+alt).attr("src", '//:0');
